...
import { selectSymbols, selectCurrentSymbol, selectFavoriteSymbols } from '../redux/selectors';
...
const SymbolBar: React.FC = () => {
    ...
    const handleFavoriteToggle = useCallback(
        (e: React.MouseEvent, symbolId: string) => {
            e.stopPropagation();
        ...
        [dispatch, favoriteSymbols]
    );

    // Memoize filtered symbols
    const filteredSymbols = useMemo(() => {
        return symbols.filter((symbol) => symbol.symbol.toLowerCase().includes(search.toLowerCase()) || symbol.name?.toLowerCase().includes(search.toLowerCase()));
    }, [symbols, search]);
