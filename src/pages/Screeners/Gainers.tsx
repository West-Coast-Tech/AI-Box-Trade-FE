// src/components/GainersTable.tsx
...
const GainersTable = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Stock Gainers Table'));
    }, [dispatch]);

    // State for gainer data, loading, and error
    const [gainers, setGainers] = useState<Gainers_Losers[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [error, setError] = useState<string>('');

    // Fetch gainers data from API
    useEffect(() => {
        const fetchGainers = async () => {
            setLoading(true);
            try {
                const response = await API.getGainers();
                setGainers(response.data);
            } catch (err: any) {
                console.error('Error fetching gainers:', err);
                setError('Failed to fetch gainers data.');
            } finally {
                setLoading(false);
            }
        };
    ...
    // Update initialRecords when gainers or sortStatus changes
    useEffect(() => {
        let sortedRecords = sortBy(gainers, sortStatus.columnAccessor as keyof Gainers_Losers);
    ...
    useEffect(() => {
        const filtered = gainers.filter((item) => {
            return (
    ...
    return (
        <div>
            <div className="panel mt-6">
                <h5 className="font-semibold text-lg dark:text-white-light mb-5">Stock Gainers</h5>
                            ...
                            columns={cols.map((col) => {
                                            ...
                                            return (
                                                <div className="flex ">
                                                    ...
                                                    <div className="w-[10vh] md:[15vh] rounded-full h-4 p-1 bg-dark-light overflow-hidden shadow-3xl dark:shadow-none dark:bg-dark-light/10">
                                                        <div
                                                            className={`bg-gradient-to-r from-green-500 to-green-300 w-full h-full rounded-full relative before:absolute before:inset-y-0 ltr:before:right-0.5 rtl:before:left-0.5 before:bg-white before:w-2 before:h-2 before:rounded-full before:m-auto`}
                                    ...
                                    return {
                                        ...
                                        render: ({ dayChangePercent }: Gainers_Losers) => (
                                            <span className="text-success">
                                                {' '}
                                                <FontAwesomeIcon icon={faArrowUp} /> {formatPercentage(dayChangePercent)}
                                            </span>
                                        ),
                                    };
                                }
                                if (col.accessor === 'dayChange') {
                                    return {
                                        ...
                                        sortable: true,
                                        hidden: isHidden,
                                        render: ({ dayChange }: Gainers_Losers) => <span className="text-success">+ {dayChange?.toFixed(2)}</span>,
                                    };
                                }
