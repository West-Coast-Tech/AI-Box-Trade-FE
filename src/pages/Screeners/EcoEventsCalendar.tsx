...
import { IEcoEventCalendar } from '../../redux/types';
...
type Column = {
    accessor: string;
    title: string;
    render?: (record: IEcoEventCalendar['ecoEvents'][0]) => React.ReactNode;
    isHidden?: boolean;
};
...
const columns: Column[] = [
    ...
    },
    {
        accessor: 'actual',
        title: 'Actual',
        render: (record) => <div className={`${record?.consensus ? (record?.actual > record?.consensus ? 'text-green-500' : 'text-red-500') : ''}`}>{record.actual}</div>,
    },
    {
        accessor: 'consensus',
        title: 'Forecast',
...
const EcoEventCalendar = () => {
    const dispatch = useDispatch();
    const [data, setData] = useState<IEcoEventCalendar[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [error, setError] = useState<string>('');
    const [activeTab, setActiveTab] = useState<string | null>('today'); // Adjusted type
    const [search, setSearch] = useState<string>(''); // Search state (optional)

    useEffect(() => {
        dispatch(setPageTitle('EcoEvents Calendar'));
    }, [dispatch]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await API.getEcoEventCalendar();
                // Ensure data is sorted by date ascending
                const sortedData = sortBy(response.data, (item) => new Date(item.date).getTime());
                setData(sortedData);
            } catch (err: any) {
                console.error(`Error fetching eco event data:`, err);
                setError(`Failed to fetch eco event calendar data.`);
            } finally {
                setLoading(false);
            }
        };
    ...
    const getFilteredData = (): GroupedData<IEcoEventCalendar['ecoEvents'][0]>[] => {
        ...
        let filtered: IEcoEventCalendar[] = [];
        ...
        if (search.trim() !== '') {
            filtered = filtered.filter((item) => item.ecoEvents.some((ecoEvent) => Object.values(ecoEvent).some((value) => String(value).toLowerCase().includes(search.toLowerCase()))));
        }
        ...
        filtered.forEach((item) => {
            ...
            groupedDataMap[formattedDate].push(...item.ecoEvents);
        });

        // Convert the map to an array of grouped data
        let groupedData: GroupedData<IEcoEventCalendar['ecoEvents'][0]>[] = Object.entries(groupedDataMap).map(([date, records]) => ({
            date,
            records,
        }));
    ...
    return (
        <div className="items-center justify-center flex">
            <div className="panel">
                <h5 className="font-semibold text-lg dark:text-white-light mb-5">Economic Events Calendar</h5>
                ...
                <EconomicCalendar<IEcoEventCalendar['ecoEvents'][0]>
                    groupedData={groupedData}
                    columns={columns}
                    title={`EcoEvents - ${activeTab ? activeTab.charAt(0).toUpperCase() + activeTab.slice(1) : 'All'}`}
                    loading={loading}
                    apiError={error}
                />
            </div>
...
export default EcoEventCalendar;
